...
def get_errors(sample_complexity, X, y, lamb, seeds=10, 
               trick=False, loss='l2', task='regression'):
    '''
    ...
    for i in range(seeds):
        ...
        # If don't want trick, take test to be the complement of train
        if not trick:
            mask = np.zeros(X.shape[0], dtype=bool)
            mask[inds] = True
            # restric to test set only
            X_test = X[~mask, :]
            y_test = y[~mask]
...
def simulate(sc_range = np.linspace(0.1, 2.5, 10), lamb = 0.01, 
             seeds = 10, trick=False, *, X, y, loss, task):
    '''
    Do kernel regression and return error.
    ...
    for alpha in sc_range:
        ...
        et, et_std, eg, eg_std = get_errors(alpha, X, y, lamb, seeds=seeds, 
                                            trick=trick, task=task, loss=loss)
